...
#         # check if the box is already square. if not, make it a square
#         diff = box_height - box_width
#         delta = int(abs(diff)/2)

#         if diff == 0:  # already square
#             return box
...
#     def move_box(box, offset):
#         '''move the box to direction specified by vector'''
#         left_x = box[0] + offset[0]
#         top_y = box[1] + offset[1]
#         right_x = box[2] + offset[0]
#         bottom_y = box[3] + offset[1]
...
#         '''
#         Find the facial landmarks in an image from the face
...
#         # actual detection
#         prediction = model.signatures["predict"](
#             tf.constant([face_img], dtype=tf.uint8))

#         # convert predictions to landmarks
#         marks = np.array(prediction['output']).flatten()[:136]
#         marks = np.reshape(marks, (-1, 2))
...
# img = cv2.imread('.\faces\images (11).jpg')
# mark_detector = MarkDetector()
# mark_detector.draw_marks(img, mark)
